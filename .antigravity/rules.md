# SOI AGENTS (Always-On Rules for Antigravity)

이 파일은 SOI 작업에서 Antigravity가 항상 적용해야 하는 최소 강제 규칙이다.
...
- 책임 분리:
  ...
  - `lib/views`: 화면/UI
- API 계약 변경 처리 순서:
  1. 영향 탐지: `./.antigravity/skills/api-lib-sync/scripts/api_change_impact.sh <repo-root>`
