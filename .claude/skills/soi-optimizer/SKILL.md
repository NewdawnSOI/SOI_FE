...
name: soi-optimizer
description: >
  SOI Flutter 프로젝트의 코드 성능 최적화 전문 스킬.
...
- 고위험 파일(`CLAUDE.md §8` 목록)인지 확인한다. 해당하면 "이 파일은 고위험 파일입니다. 변경 시 주의 사항을 먼저 명시하겠습니다."라고 선언한다.
- Serena 도구(`mcp__plugin_serena_serena__*`)를 우선 사용하고, 단순 경로 검색은 Glob/Grep으로 처리한다.
...
**캐시 및 상태 관리**
- Controller의 TTL 정책이 `CLAUDE.md §6`과 일치하는가?
...
**미디어 성능**
- 이미지 캐시 크기가 `CLAUDE.md §5` 기준(debug: 50개/50MB, release: 30개/30MB)을 초과하는가?
...
복잡한 변경이 여러 파일에 걸친다면 서브에이전트(Agent 도구)를 분야별로 나눠서 병렬 실행한다.

### 4단계: 코드 수정

- 아키텍처 경계(`CLAUDE.md §3`)를 지킨다: 생성 코드(`api/generated/**`)는 절대 수동 수정 금지.
...
변경 범위가 넓을 때 병렬 처리로 시간을 절약한다:

- **분석 단계**: 여러 파일을 동시에 읽어야 할 때 → 파일별 서브에이전트 병렬 실행
- **수정 단계**: 서로 독립적인 파일 수정 → 각 파일을 서브에이전트에 위임
- **테스트 단계**: 여러 테스트 파일 작성 → 병렬 작성 후 수집

서브에이전트에게 항상 전달해야 할 컨텍스트:
1. 이 파일은 SOI Flutter 프로젝트 (Provider + ChangeNotifier 아키텍처)
2. `CLAUDE.md`의 캐시 정책 (`§5`, `§6`)
3. 고위험 파일 여부
4. 수정 목적과 범위
