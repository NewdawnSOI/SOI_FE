...
- Codex에서는 로컬 코드 탐색을 우선한다. 빠른 텍스트/경로 검색은 `rg`, 여러 파일 조회는 `multi_tool_use.parallel`, 수동 편집은 `apply_patch`를 사용한다.
...
복잡한 변경이 여러 파일에 걸친다면, 기본값은 현재 Codex가 직접 순차 처리한다. 사용자가 명시적으로 병렬 에이전트/위임을 요청한 경우에만 Agent 도구를 분야별로 나눠 사용한다.
...
2. `AGENTS.md`의 캐시 정책 (`§5`, `§6`)
3. 고위험 파일 여부
4. 수정 목적과 범위
5. 다른 에이전트의 변경을 되돌리지 말고, 맡은 파일/범위만 책임질 것
